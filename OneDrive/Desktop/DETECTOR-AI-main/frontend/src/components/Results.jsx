...
export default function Results({ result, onReset }) {
  ...
  return (
    <div className="reveal-slow fade-in-slow" style={{ animation: "fadeInUpCascade 600ms cubic-bezier(0.34,1.56,0.64,1)" }}>
      <style>{`
        @keyframes fadeUp { from{opacity:0;transform:translateY(16px)} to{opacity:1;transform:translateY(0)} }
        .toggle-btn { background:none; border:1px solid var(--border2); border-radius:6px;
          padding:6px 12px; color:var(--muted2); font-family:var(--mono); font-size:11px;
          cursor:pointer; letter-spacing:1px; transition:all 0.3s cubic-bezier(0.34,1.56,0.64,1);
          background: linear-gradient(135deg, rgba(8,14,22,0.95), rgba(10,18,28,0.72)); 
        ...
        .toggle-btn:hover { border-color:var(--accent); color:var(--accent); 
          box-shadow:0 0 0 1px rgba(0,229,255,0.25) inset, 0 0 12px rgba(0,229,255,0.15);
          transform: translateY(-1px); }
        .toggle-btn:hover::before { animation: glassShine 0.6s ease-in-out; opacity: 1; }
        .toggle-btn:active { transform: translateY(0); }
      `}</style>

      {/* ── Main Verdict Card ── */}
      <div className="hud-frame fade-in-cascade-1" style={{
        animation: "fadeInUpCascade 500ms cubic-bezier(0.34,1.56,0.64,1)",
        marginBottom: 16,
      }}>
      <div style={{
        ...
        <div style={{ position:"absolute", inset:0, background:color, opacity:0.04, animation: "hudShine 3s ease-in-out infinite" }} />
        <div style={{ display:"flex", gap:24, alignItems:"center", flexWrap:"wrap", position: "relative" }}>
          ...
          <div style={{ flex:1, minWidth: 200 }}>
            ...
            <div style={{ fontSize:11, color:"var(--muted)", margin:"10px 0 4px", transition: "all 200ms ease" }}>
              Confidence: <strong style={{ color:"var(--text)" }}>{result.confidence}</strong>
              {result.id && typeof result.id === "string" && (
                <span style={{ marginLeft:12, color:"var(--muted)", animation: "fadeInUpCascade 600ms ease-out 300ms backwards" }}>
                  ID: <code style={{ color:"var(--accent)", fontSize:10, fontFamily: "var(--mono)" }}>{result.id.slice(0,8)}</code>
                </span>
              )}
            </div>
            <div style={{
              ...
              animation: "fadeInUpCascade 600ms ease-out 200ms backwards"
            }}>
              {result.summary}
            </div>
          </div>
        ...
        {result.stat_ai_score !== undefined && result.llm_ai_score !== undefined && (
          <div style={{
            ...
            animation: "fadeInUpCascade 700ms ease-out 400ms backwards"
          }}>
            <div>
              <div style={{ fontSize:10, color:"var(--muted)", letterSpacing:1, marginBottom:6 }}>
                <RealisticFigure symbol="📊" className="animated-emoji emoji-status" /> STATISTICAL ANALYSIS
              </div>
              <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                <div style={{
                  ...
                  <div style={{
                    ...
                    borderRadius:4, transition:"width 1.2s cubic-bezier(0.4,0,0.2,1)", boxShadow: `0 0 8px ${result.stat_ai_score > 60 ? "var(--accent2)" : "var(--accent3)"}`
                  }} />
                </div>
                <span style={{ fontSize:13, fontWeight:700, minWidth: 45,
                  color: result.stat_ai_score > 60 ? "var(--accent2)" : "var(--accent3)", animation: "blurInReveal 600ms ease-out 500ms backwards" }}>
            ...
            <div>
              <div style={{ fontSize:10, color:"var(--muted)", letterSpacing:1, marginBottom:6 }}>
                <RealisticFigure symbol="🔬" className="animated-emoji emoji-status" /> LLM ENSEMBLE (3 MODELS)
              </div>
              <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                <div style={{
                  flex:1, height:8, background:"var(--bg)", borderRadius:4, overflow:"hidden",
                  ...
                  <div style={{
                    ...
                    borderRadius:4, transition:"width 1.2s cubic-bezier(0.4,0,0.2,1)", boxShadow: `0 0 8px ${result.llm_ai_score > 60 ? "var(--accent2)" : "var(--accent3)"}`
                  }} />
                </div>
                <span style={{ fontSize:13, fontWeight:700, minWidth: 45,
                  color: result.llm_ai_score > 60 ? "var(--accent2)" : "var(--accent3)", animation: "blurInReveal 600ms ease-out 550ms backwards" }}>
                  {result.llm_ai_score}%
                </span>
              </div>
            </div>
      ...
      {/* ── Stats Row ── */}
      <div className="hud-frame fade-in-cascade-2" style={{ marginBottom:16, animation: "fadeInUpCascade 500ms cubic-bezier(0.34,1.56,0.64,1) 100ms backwards" }}>
      <Card>
        ...
        <div style={{
          ...
          {[
            ...
            <div key={label} style={{ textAlign:"center", animation: `fadeInUpCascade 500ms ease-out ${200 + idx * 80}ms backwards` }}>
              <div style={{ fontFamily:"var(--head)", fontSize:22, fontWeight:800, color:c, transition: "all 200ms ease" }}>{val}</div>
      ...
      {/* ── Signal Bars ── */}
      {result.signals && (
        <div className="hud-frame fade-in-cascade-3" style={{ marginBottom:16, animation: "fadeInUpCascade 500ms cubic-bezier(0.34,1.56,0.64,1) 200ms backwards" }}>
        <Card>
          <CardHeader>Detection Signals</CardHeader>
          <div style={{ padding:16, display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
            {Object.entries(result.signals).map(([key, sig], idx) => {
              ...
              return (
                <div key={key} style={{
                  ...
                  borderRadius:8, padding:12, transition: "all 300ms ease",
                  animation: `fadeInUpCascade 500ms ease-out ${300 + idx * 60}ms backwards`,
                  cursor: "pointer",
                }}>
                  <div style={{ fontSize:10, color:"var(--muted)", letterSpacing:1, textTransform:"uppercase", marginBottom:8, display:"flex", gap:6, alignItems:"center" }}>
                    <span style={{ width:6, height:6, borderRadius:"50%", background:barColor, display:"inline-block", flexShrink:0, boxShadow: `0 0 8px ${barColor}` }} />
                    {label}
                  </div>
                  <div style={{ background:"var(--surface)", borderRadius:3, height:5, marginBottom:6, overflow:"hidden", boxShadow: "inset 0 2px 4px rgba(0,0,0,0.5)" }}>
                    <div style={{ width:`${sig.score}%`, height:"100%", background:barColor, borderRadius:3, transition:"width 1.2s cubic-bezier(0.4,0,0.2,1)", boxShadow: `0 0 12px ${barColor}` }} />
                  </div>
                  <div style={{ display:"flex", justifyContent:"space-between", fontSize:10, color:"var(--muted)" }}>
                    <span style={{ fontSize:10 }}>{sig.label}</span>
                    <span style={{ fontWeight:700, color:"var(--text)", animation: "blurInReveal 600ms ease-out " + (300 + idx * 60 + 200) + "ms backwards" }}>{sig.score}/100</span>
                  </div>
                </div>
              );
            })}
      ...
      {/* ── Forensic Analysis ── */}
      <div className="hud-frame" style={{ marginBottom:16 }}>
      <Card>
        <CardHeader>Forensic Analysis</CardHeader>
        <div style={{ padding:20 }}>
          {result.ai_flags?.length > 0 && (
          <Section title={<><RealisticFigure symbol="🤖" className="animated-emoji emoji-status" /> AI Signals</>} color="var(--accent2)">
              <FlagList items={result.ai_flags} color="var(--accent2)" />
            </Section>
          )}
          {result.human_flags?.length > 0 && (
          <Section title={<><RealisticFigure symbol="🧑" className="animated-emoji emoji-status" /> Human Signals</>} color="var(--accent3)">
              <FlagList items={result.human_flags} color="var(--accent3)" />
            </Section>
          )}
          {result.suspicious_phrases?.length > 0 && (
            <Section title={<><RealisticFigure symbol="🚩" className="animated-emoji emoji-status" /> Flagged Phrases</>} color="var(--warn)" last>
